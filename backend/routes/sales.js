...
// CREAR VENTA
// Transaccional + bloqueo de fila (FOR UPDATE) para evitar que dos ventas
// simultáneas dejen el stock en negativo, y el precio se toma siempre de
// la base de datos (nunca del cliente) para evitar manipulación.
router.post(
  "/sales",
  auth,
  role(["admin", "vendedor"]),
  async (req, res) => {
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (!row) {
          ...
          return res.status(400).json({
            message: `Producto ${item.id} no existe en esta sede`,
          });
        }
        ...
        if (row.stock < item.quantity) {
          await conn.rollback();
          return res.status(400).json({
            message: `Stock insuficiente para ${row.name}`,
          });
        }
      ...
      const [saleResult] = await conn.query(
        "INSERT INTO sales (total, branch_id) VALUES (?, ?)",
        [total, req.user.branch_id]
      );
...
// GET SALES
// Una sola query con JOIN (en vez de una query por venta) para evitar N+1.
router.get(
  "/sales",
  auth,
  role(["admin", "vendedor"]),
