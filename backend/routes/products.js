...
// GET PRODUCTS — todos los productos de la sede, incluidos los de stock 0.
// Antes el WHERE bi.branch_id = ? dentro del JOIN filtraba filas nulas y
// ocultaba productos sin entrada en branch_inventory. Ahora el filtro
// va en el ON del LEFT JOIN, así products siempre aparece.
router.get(
  "/products",
  auth,
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await conn.query("DELETE FROM branch_inventory WHERE product_id = ?", [id]);
      ...
      if (err.code === "ER_ROW_IS_REFERENCED_2" || err.code === "ER_ROW_IS_REFERENCED")
        return res.status(409).json({ error: "No se puede eliminar: el producto tiene ventas o movimientos asociados" });
